module MatDeforNeohookeanADCModule
...
"""
	MatDeforNeohookeanADC{MR<:AbstractDeforModelRed, MTAN<:Function, MUPD<:Function} <: AbstractMatDeforNonlinear

Type for triaxial neohookean hyperelastic material.

"""
struct  MatDeforNeohookeanADC{MR<:AbstractDeforModelRed, MTAN<:Function, MUPD<:Function} <: AbstractMatDeforNonlinear
...
"""
    MatDeforNeohookeanADC(mr::Type{MR}, E::FFlt, nu::FFlt) where {MR}

Create neohookean isotropic elastic material.

The mass density is the default value.
"""
function MatDeforNeohookeanADC(mr::Type{MR}, E::FFlt, nu::FFlt) where {MR}
	mass_density = 1.0
	return MatDeforNeohookeanADC(mr, mass_density, E, nu)
end
...
"""
    MatDeforNeohookeanADC(mr::Type{DeforModelRed3D}, mass_density::FFlt, E::FFlt, nu::FFlt)

Create triaxial neohookean hyperelastic material.
"""
function MatDeforNeohookeanADC(mr::Type{DeforModelRed3D}, mass_density::FFlt, E::FFlt, nu::FFlt)
	function tangentmoduli3d!(self::MatDeforNeohookeanADC, D::FFltMat, statev::FFltVec, Fn1::FFltMat, Fn::FFltMat, tn::FFlt, dtn::FFlt, loc::FFltMat, label::FInt)
		C = Fn1'*Fn1;
		Cv = fill(0.0, 6)
		strainttov!(mr, Cv, C)
	...
	function update3d!(self::MatDeforNeohookeanADC, statev::FFltVec, cauchy::FFltVec, output::FFltVec, Fn1::FFltMat, Fn::FFltMat, tn::FFlt, dtn::FFlt, loc::FFltMat=zeros(3,1), label::FInt=0, quantity=:nothing)
	...
	_mu = E / 2. / (1+nu);
	return MatDeforNeohookeanADC(mr, mass_density, E, nu, _lambda, _mu,
...
"""
    MatDeforNeohookeanADC(mr::Type{DeforModelRed2DStrain}, mass_density::FFlt, E::FFlt, nu::FFlt)

Create triaxial neohookean hyperelastic material.
"""
function MatDeforNeohookeanADC(mr::Type{DeforModelRed2DStrain}, mass_density::FFlt, E::FFlt, nu::FFlt)
	function tangentmoduli2dstrain!(self::MatDeforNeohookeanADC, D::FFltMat, statev::FFltVec, Fn1::FFltMat, Fn::FFltMat, tn::FFlt, dtn::FFlt, loc::FFltMat, label::FInt)
		Fn13d = fill(0.0, 3, 3)
		Fn13d[1:2, 1:2] = Fn1
		Fn13d[3, 3] = 1.0
		C = Fn13d'*Fn13d;
	...
	function update2dstrain!(self::MatDeforNeohookeanADC, statev::FFltVec, cauchy::FFltVec, output::FFltVec, Fn1::FFltMat, Fn::FFltMat, tn::FFlt, dtn::FFlt, loc::FFltMat=zeros(3,1), label::FInt=0, quantity=:nothing)
		@assert length(cauchy) == nstressstrain(self.mr)
		Fn13d = fill(0.0, 3, 3)
		Fn13d[1:2, 1:2] = Fn1
		Fn13d[3, 3] = 1.0
		...
		S = fill(0.0, 3, 3)
		stressvtot!(D3D, S, Sv)
		cauchyt = Fn13d*(S/det(Fn1))*Fn13d'; # Cauchy stress, 3D
		stressttov!(mr, cauchy, cauchyt) # 2D stress vector
		if quantity == :nothing
			#Nothing to be copied to the output array
		elseif quantity == :cauchy || quantity == :Cauchy
			(length(output) >= 3) || (output = zeros(3)) # make sure we can store it
